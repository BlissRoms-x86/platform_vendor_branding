...
func init() {
	android.RegisterModuleType("lmodroid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lmodroidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lmodroidExpandVariables(ctx, dep_file)
	...
	cmd := lmodroidExpandVariables(ctx, String(g.properties.Cmd))
