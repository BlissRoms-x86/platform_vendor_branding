...
function setup_vendor() {
    ...
    export LMODROID_ROOT="$3"
    if [ ! -d "$LMODROID_ROOT" ]; then
        echo "\$LMODROID_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$LMODROID_ROOT/$OUTDIR" ]; then
        mkdir -p "$LMODROID_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$LMODROID_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$LMODROID_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$LMODROID_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$LMODROID_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) 2020 The LineageOS Project\n" >> $1
        printf " * Copyright (C) $YEAR The LibreMobileOS Foundation\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-2020 The LineageOS Project\n" >> $1
        printf " * Copyright (C) 2020-$YEAR The LibreMobileOS Foundation\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-2020 The LineageOS Project\n" >> $1
        printf " * Copyright (C) 2020-$YEAR The LibreMobileOS Foundation\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2020 The LibreMobileOS Foundation\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) 2020 The LineageOS Project\n" >> $1
            printf "# Copyright (C) $YEAR The LibreMobileOS Foundation\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-2020 The LineageOS Project\n" >> $1
            printf "# Copyright (C) 2020-$YEAR The LibreMobileOS Foundation\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-2020 The LineageOS Project\n" >> $1
            printf "# Copyright (C) 2020-2020 The LibreMobileOS Foundation\n" >> $1
        fi
    else
        printf "# Copyright (C) 2020 The LineageOS Project\n" > $1
        printf "# Copyright (C) $YEAR The LibreMobileOS Foundation\n" > $1
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$LMODROID_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$LMODROID_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$LMODROID_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$LMODROID_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$LMODROID_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$LMODROID_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$LMODROID_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$LMODROID_ROOT"/vendor/lmodroid/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$LMODROID_ROOT"/"$OUTDIR"/radio
